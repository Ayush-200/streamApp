...
const ScheduleMeeting = () => {
  ...
  const handleSubmit = async(e) =>{
    ...
    try {
      ...
      const emailId = user.email;
      const response = await fetch(`${import.meta.env.VITE_APP_URL}/addUsersMeetings/${emailId}`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchMeetings = async() => {
      const response = await fetch(`${import.meta.env.VITE_APP_URL}/getUserMeetings/${emailId}`);
      const data = await response.json();
      console.log("data ")
      console.log(data);
      const combined = (data.meeting || []).map((m, i) =>({
